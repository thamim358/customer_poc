...
import { classNames, profileName } from "src/utilities";
import { Image } from ".";
...
export const ProfileTopNavbar = ({
    ...
    const { currentPractitioner } = useSelector((state) => state?.practitionerState);
    const [appointmentwithColors, setappointmentwithColors] = useState({});
    const [imgError, setImgError] = useState(false);
    const loadedList = getAppointmentStatusList?.length >0 ? getAppointmentStatusList : encounterList?.length > 0 ? encounterList : null

    useEffect(() => {
            const newappointmentwithColors = { ...appointmentwithColors };
            if(loadedList?.length >0 ){
                loadedList?.forEach((appointment) => {
                    const name = `${appointment?.practitioner_firstName ? appointment?.practitioner_firstName :appointment?.practitioner_first_name} ${
    ...
    let fullName = currentPractitioner.practitioner_name;
        ...
        <>
            {base64Image && !imgError ? (
                ...
                <div
                    ...
                    style={{
                        background:  appointmentwithColors[name] || generateColor(nameWithoutTitle || "") ,
                    }}
                                    >
                  {generateColor(nameWithoutTitle) && name && profileName(name)}
                </div>
            )}
        </>
    );
